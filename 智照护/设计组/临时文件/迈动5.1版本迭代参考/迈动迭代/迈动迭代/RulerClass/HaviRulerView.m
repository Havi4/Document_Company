...
#define SHEIGHT 8//中间指示器的闭合三角形的高度
#define INDICATORCOLOR [UIColor redColor].CGColor // 中间指示器颜色

@implementation HaviRulerView {
    HaviRulerScrollView * rulerScrollView;
}
...
- (HaviRulerScrollView *)rulerScrollView {
    HaviRulerScrollView * rScrollView = [HaviRulerScrollView new];
    rScrollView.delegate = self;
    rScrollView.showsVerticalScrollIndicator = NO;
    return rScrollView;
}
...
- (void)scrollViewDidScroll:(HaviRulerScrollView *)scrollView {
    CGFloat offSetY = scrollView.contentOffset.y + self.frame.size.height / 2 - DISTANCELEFTANDRIGHT;
    ...
    if (self.rulerDeletate) {
        ...
        scrollView.mode = NO;
        [self.rulerDeletate rulerSelected:scrollView];
    }
}

- (void)scrollViewDidEndDecelerating:(HaviRulerScrollView *)scrollView {
    [self animationRebound:scrollView];
}

- (void)scrollViewDidEndDragging:(HaviRulerScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    [self animationRebound:scrollView];
}

- (void)animationRebound:(HaviRulerScrollView *)scrollView {
    CGFloat offSetY = scrollView.contentOffset.y + self.frame.size.height / 2 - DISTANCELEFTANDRIGHT;
    CGFloat oX = (offSetY / DISTANCEVALUE) * [scrollView.rulerAverage floatValue];
#ifdef DEBUG
...
#endif
    CGFloat offX = (oX / ([scrollView.rulerAverage floatValue])) * DISTANCEVALUE + DISTANCELEFTANDRIGHT - self.frame.size.height / 2;
    [UIView animateWithDuration:.2f animations:^{
        scrollView.contentOffset = CGPointMake(0, offX);
    }];
}
...
//    [self.layer addSublayer:gradient];
    ...
    CGPathMoveToPoint(pathLine, NULL,  self.frame.size.width - DISTANCETOPANDBOTTOM - ruleHeight,self.frame.size.height / 2);
    CGPathAddLineToPoint(pathLine, NULL, DISTANCETOPANDBOTTOM + SHEIGHT ,self.frame.size.height / 2);
    
    CGPathAddLineToPoint(pathLine, NULL,DISTANCETOPANDBOTTOM,  self.frame.size.height / 2 - SHEIGHT / 2);
    CGPathAddLineToPoint(pathLine, NULL, DISTANCETOPANDBOTTOM, self.frame.size.height / 2 + SHEIGHT / 2);
    CGPathAddLineToPoint(pathLine, NULL,  DISTANCETOPANDBOTTOM + SHEIGHT,self.frame.size.height / 2);
